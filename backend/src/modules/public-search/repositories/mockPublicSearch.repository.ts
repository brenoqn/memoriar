import type { PublicDeceasedDetail, PublicDeceasedSearchItem, MovementType } from '@memoriar/shared';
...
const MOCK_ITEMS: PublicDeceasedSearchItem[] = [
  {
...
export class MockPublicSearchRepository implements PublicSearchRepository {
  async search(query: PublicSearchQuery): Promise<PublicDeceasedSearchItem[]> {
    const q = query.q.toLowerCase();
    return MOCK_ITEMS.filter((i) => {
      const matchesText =
        i.deceased_name.toLowerCase().includes(q) ||
        (i.public_reference || '').toLowerCase().includes(q) ||
        (i.location_code || '').toLowerCase().includes(q);
      const matchesCemetery = query.cemeteryId ? i.cemetery_id === query.cemeteryId : true;
      return matchesText && matchesCemetery;
    });
  }

  async detail(query: PublicSearchDetailQuery): Promise<PublicDeceasedDetail | null> {
    const item = MOCK_ITEMS.find((i) => i.burial_case_id === query.burialCaseId);
    if (!item) return null;
    const last_movement_type: MovementType | null = item.current_location_id ? 'interment' : null;
    return {
      ...item,
      last_movement_type,
